...
const axiosDefaults = {
  baseURL: 'http://192.168.254.108:1337/api',
  timeout: 5000,
  headers: {
    'Content-Type': 'application/json',
  },
};
export const imageuri = 'http://192.168.254.108:1337/public/images/';
export const fileuri = 'http://192.168.254.108:1337/public/files/';
...
async function setUserAccessToken(token: string): Promise<void> {
  return AsyncStorage.setItem('userAccessToken', token);
}
  ...
  async (error: AxiosError) => {
    if (error.response && error.response.status === 401) {
      ...
      if (userRefreshToken) {
        try {
          console.log(
            'ProtectedInstance response interceptor calling refresh token',
          );
